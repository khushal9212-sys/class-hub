...
# This key must come from an environment variable (Render -> Environment tab),
# never hardcoded here, since it bypasses all RLS security rules.
...
# All of these use supabase_admin (service_role key) so RLS doesn't block the delete.
...
def admin_delete_poll(poll_id):
    ...
    try:
        supabase_admin.table("poll_options").delete().eq("poll_id", poll_id).execute()
        supabase_admin.table("polls").delete().eq("id", poll_id).execute()
...
def admin_delete_date(date_id):
    ...
    try:
        ...
        supabase_admin.table("date_attachments").delete().eq("date_id", date_id).execute()
        supabase_admin.table("important_dates").delete().eq("id", date_id).execute()
...
def admin_delete_attachment(attachment_id):
    ...
    try:
        ...
        supabase_admin.table("date_attachments").delete().eq("id", attachment_id).execute()
        return redirect(request.referrer or url_for("home"))
    except Exception as e:
        print(f"❌ Delete error: {e}")
        return redirect(request.referrer or url_for("admin"))
...
def admin_delete_all_files():
    ...
    try:
        ...
        for file in files:
            ...
            if file_url:
                delete_from_storage(file_url)
        supabase_admin.table("files").delete().neq("id", 0).execute()
        return redirect(url_for("admin"))
    except Exception as e:
        print(f"❌ Delete error: {e}")
        return redirect(url_for("admin"))
...
def admin_delete_all_polls():
    ...
    try:
        ...
        for poll in polls:
            supabase_admin.table("poll_options").delete().eq("poll_id", poll["id"]).execute()
        supabase_admin.table("polls").delete().neq("id", 0).execute()
        return redirect(url_for("admin"))
    except Exception as e:
        print(f"❌ Delete error: {e}")
        return redirect(url_for("admin"))
...
def admin_delete_all_announcements():
    ...
    try:
        ...
        for announcement in announcements:
            ...
            if file_url:
                delete_from_storage(file_url)
        supabase_admin.table("announcements").delete().neq("id", 0).execute()
        return redirect(url_for("admin"))
    except Exception as e:
        print(f"❌ Delete error: {e}")
        return redirect(url_for("admin"))
